...
impl<T: Send + 'static> Receiver<T> {
    ...
    // Same as recv_items but returns items in reverse order
    // saves the O(n) reverse operation
    pub fn recv_items_backwards(&self) -> QueueItemIter<T> {
        self.queue.recv_items_backwards()
    }
}

impl<T: Send + 'static> RTQueue<T> {
    /// Create a new queue, and return endpoints for sending and receiving.
    pub fn new() -> (Sender<T>, Receiver<T>) {
        let queue = Arc::new(RTQueue {
    ...
    fn recv_items_backwards(&self) -> QueueItemIter<T> {
        unsafe { QueueItemIter(self.pop_all()) }
    }
...
pub fn try_queue() {
    let (tx, to_worker) = RTQueue::new();
    let (from_worker, rx) = RTQueue::new();
